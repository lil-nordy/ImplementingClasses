...
# >Big Notes: remember to reuse code. You've been rewriting methods and functions for ages. Learn how to call that
# shit.
import math
...
class Point(object):
    """ Represents 2-D point in x y space."""

    def __init__(self, x, y):
        """Consttructs a new Point object"""
        self.x = x # sets your own instance variables named x equal to an object instance's instance variable
        self.y = y
        self.start_x = x
        self.start_y = y
        self.moves = 0
    ...
    def __repr__(self):
        """Returns a strings that represents the point object"""
        # return "Point(" + str(self.x) + ", " + str(self.y) + ")"
        return "Point({}, {})".format(self.x, self.y)
        # TODO: Check out the $s way to do this via the online documentation. Reading is really important
        # I found a couple of really good url;s that I really need to checkout:
        # https://pyformat.info/
        # https://www.google.com/search?q=elif+python&rlz=1C1GCEB_enUS760US760&oq=elif+python&aqs=chrome.0.69i59.1397j0j7&sourceid=chrome&ie=UTF-8
    ...
    def move_to(self, x, y):
        """Changes the supplied point's x and y values to those supplied"""
        self.x = x
        self.y = y
        self.moves += 1
        self.distance += self.get_distance_from_start()
        # No side effects specified. Don't return anything. Just mutate or reassign.
    ...
    def move_by(self, dx, dy):
        """Moves the supplied point's x and y values by amounts dx and dy passed in respectively"""
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves += 1
        self.distance += self.get_distance_from(Point(self.x + dx, self.y + dy))
        # TODO: understand Fisher's method of checking to see if any side effects were returned by using a =

    def get_number_of_moves_made(self):
        """Returns an integer that is the number of times that the point has been moved"""
        return self.moves

    def get_distance_from(self, point):
        """returns an integer distance from the first point upon which the method is called to the second point
        supplied"""
        a = self.x - point.x
        b = self.y - point.y
        c = math.sqrt(a**2 + b**2)
    ...
    def get_distance_from_start(self):
        """Returns an integer representing the distance from where it was constructed to where it is currently"""
        a = self.x - self.start_x
        b = self.y - self.start_y
        c = math.sqrt(a**2 + b**2)
        return c
...
# > Nate Note: That's not what fisher said. He didn't want the move_by methods to have any additional side effects.
########################################################################
...
def run_test_move_to():
    ...
    # ToDo 42: understand this a little better.
    # > how exactly does the Point() function know what to do with the self.x and self.y that were...haphazardly
    # returned? Why aren't those values positional?
    # ------------------------------------------------------------------
    print()
    print('-----------------------------------------------------------')
...
def run_test_move_by():
    ...
    p2.move_by(0, 0)
    print()
    print('Expected for a: None')
    print('Actual for a: ', a)
    print('Expected for p1: Point(15, 7)')
    print('Actual for p1:  ', p1)
    print('Expected for p2: Point(50, 20)')
    print('Actual for p2:  ', p2)
